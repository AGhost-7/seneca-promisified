...
var SenecaEntityWrapper = function () {
	...
	_createClass(SenecaEntityWrapper, [{
		...
		value: function save$(obj) {
			...
			return new _anyPromise2.default(function (resolve, reject) {
				var onComplete = function onComplete(err, ent) {
					if (err) return reject(err);
					resolve(_this4._onEnt(ent));
				};

				if (obj) {
					_this4._entity.save(obj, onComplete);
				} else {
					_this4._entity.save(onComplete);
				}
			});
		}
	}, {
		...
		value: function load$(query) {
			...
			this._clearEnt();
			return new _anyPromise2.default(function (resolve, reject) {
				_this5._entity.load$(function (err, ent) {});
			});
		}
	}, {
		key: 'list$',
...
var make = function make(name, base, zone) {
	var entity = this._seneca.make(name, base, zone);
